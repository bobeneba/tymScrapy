...
from tymspider.tymspider.items import EnterpriseItem
import scrapy
...
class EnterpriseSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        self.base_url = {
           ...
           # "drug":"http://172.10.128.130:10001/zzgrid/zzgl/crowd/drug/listDataByPartyListNP.json?" #cid
            "Enterprise" : "http://172.10.128.130:10001/zzgrid/zzgl/grid/nonPublicOrg/listData.json?"
        ...
        for key,value_url in self.base_url.items():
            ...
            for code in codeList:
                ...
                url = value_url + "gridId=" + str(code[2]).strip() + "&dataStatus=001&page=1&rows=20"
    ...
    def parse_json(self, response):
        ...
        orgcode = re.findall(r"gridId=(.+)&d",url_from)
        ...
        if(int(total/1000)==0):
            url = crawl_url + "gridId=" + str(orgcode[0]).strip() + "&dataStatus=001&"
    ...
    def parse_item(self, response):
        ...
        for row in res_rows:
            ...
            item['enterpriseType'] = row['corTypeNameGb']
            item['contact'] = row['telephone']
            item['name'] = row['corName']
            item['corAddr'] = row['corAddr']
            item['respoName'] = row['contactPeople']
    ...
    def get_url(self):
        sql_code='select code ,no,cid  from per_region where length(no)=20'
