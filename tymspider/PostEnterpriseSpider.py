...
class PostEnterpriseSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        window.close()
        self.base_url = {
            "enterprise": "https://quannan.netalent.cn:8086/quannanv3/api/enterprise/save"
        }
        self.cookiedict = {}
        ...
        url = self.base_url['enterprise']
        self.headers ={}
        self.headers["Content-Type"]="application/json;charset=UTF-8"
        self.headers["Accept"]="application/json"
        self.headers["zeus-token"]=cookie_items[0]["value"]

        for (region_no,name,corAddr ,contact, respoName ) in self.data:
            api_data = {}
            api_data['regionNo'] = region_no
            api_data['type']=str("101140")
            api_data['name']=str(name)
            api_data['enterpriseType'] = str("101145")
            api_data['contact'] = str(contact)
            api_data['address'] = str(corAddr)
            api_data['respoName'] =str(respoName)
    ...
    def getdata(self):
        self.open_sqlite3()
        sql_select = "select region_no,name,corAddr , contact, respoName from enterprise "
