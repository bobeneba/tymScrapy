...
from tymspider.tymspider.items import ReleaseCriminalItem
import scrapy
...
class ReleaseCriminalSpider(scrapy.Spider):

    name = "ReleaseCriminal"
    ...
    def parse_item(self, response):
        ...
        for row in res_rows:
            ...
            item["idcard"] = row["identityCard"]
            item["name_person"] = row["name"]


            item['accusationType'] = row['accusationLabel']
            item["sentence"] = row["originSentenced"]
            item["releaseDate"]=row["releaseDateStr"]
            item["servingPlace"]=row["detentionPlaces"]
            item["dangerEstimateType"]=row["riskAssessmentLabel"]
            item["connectDate"]=row["joinDateStr"]
            item["connectCondition"] = "100469"
            item["settleDate"] = row["placementDateStr"]
            item['settleCondition']= row['placementSituationLabel']
            item["unsettleReason"] = "0"
            item["educate"] = "0"
            item["isRecidivist"] = "0"
            item['isRepeat'] = "0"
